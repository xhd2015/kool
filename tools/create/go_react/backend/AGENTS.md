...
## Frontend: API Calls Stay Under `PROJECT_NAME-react/src/api`

Do not call `fetch`, `apiFetch`, `new EventSource`, or `apiEventSource` outside `PROJECT_NAME-react/src/api`.

All frontend API transport and endpoint wrappers must live under `PROJECT_NAME-react/src/api`. Components, hooks, and feature pages should import typed API functions from that directory instead of constructing API requests directly.

Before finishing frontend API changes, verify this rule with:

```sh
rg -n "\bfetch\(|\bapiFetch\b|new EventSource|\bapiEventSource\b" PROJECT_NAME-react/src --glob '!PROJECT_NAME-react/src/api/**'
```
