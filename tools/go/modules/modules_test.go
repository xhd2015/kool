...
func TestUpdateLocalDepsAndRender(t *testing.T) {
	...
	for _, want := range []string{
		"example.com/root.git/cli v0.0.2",
	...
	if gitTagExists(origin, "types/v0.0.2") {
		t.Fatalf("unchanged types should not create types/v0.0.2")
	}
	for _, tag := range []string{"cli/v0.0.2", "v0.0.2"} {
		mustRunGit(t, origin, "rev-parse", "--verify", "refs/tags/"+tag)
	}
	if !strings.Contains(buf.String(), "new tag: cli/v0.0.1 -> cli/v0.0.2") {
		t.Fatalf("rendered tree missing cli tag annotation:\n%s", buf.String())
	}
	...
	if !strings.Contains(buf.String(), "updated: cli/go.mod v0.0.0 -> v0.0.2, replace removed") {
...
func TestUpdateLocalDepsAndRenderDryRun(t *testing.T) {
	...
	localTagsBefore := mustRunGitOutput(t, dir, "tag", "-l")
	originTagsBefore := mustRunGitOutput(t, origin, "tag", "-l")
	...
	if tags := mustRunGitOutput(t, dir, "tag", "-l"); tags != localTagsBefore {
		t.Fatalf("dry-run created local tags:\nbefore:\n%s\nafter:\n%s", localTagsBefore, tags)
	}
	if tags := mustRunGitOutput(t, origin, "tag", "-l"); tags != originTagsBefore {
		t.Fatalf("dry-run pushed tags:\nbefore:\n%s\nafter:\n%s", originTagsBefore, tags)
	}

	for _, want := range []string{
		"updated: types/go.mod v0.0.0 -> v0.0.1, replace removed",
		"new tag: cli/v0.0.1 -> cli/v0.0.2",
		"updated: cli/go.mod v0.0.0 -> v0.0.2, replace removed",
		"new tag: v0.0.1 -> v0.0.2",
	} {
		if !strings.Contains(buf.String(), want) {
...
func TestUpdateLocalDepsSkipsUntaggedModule(t *testing.T) {
	...
	if tags := mustRunGitOutput(t, origin, "tag", "-l"); tags != "" {
		t.Fatalf("expected no tags created, got:\n%s", tags)
	}
	if status := mustRunGitOutput(t, dir, "status", "--short"); status != "" {
		t.Fatalf("update left uncommitted changes:\n%s", status)
	}
...
func TestUpdateLocalDepsSkipsUntaggedModuleDryRun(t *testing.T) {
	...
	if rootGoMod := mustRead(t, filepath.Join(dir, "go.mod")); rootGoMod != rootGoModBefore {
		t.Fatalf("dry-run changed go.mod")
	}
	if headAfter := mustRunGitOutput(t, dir, "rev-parse", "HEAD"); headAfter != headBefore {
		t.Fatalf("dry-run changed HEAD: before %s after %s", headBefore, headAfter)
	}
...
`)
	mustRunGit(t, dir, "add", ".")
	mustRunGit(t, dir, "commit", "-m", "initial")
	for _, tag := range []string{"v0.0.1", "types/v0.0.1", "cli/v0.0.1"} {
