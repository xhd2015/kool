...
**Feature**: kool go replace adds a filesystem replace directive for a local module

```
# consumer module with require -> replace target dir -> replace mod=absDir in cwd
user -> kool go replace <dir> -> go_replace.Replace -> go.mod edited in cwd
```
...
3. Run replace via library or CLI
4. Assert consumer go.mod contains the expected replace directive
...
func initConsumerModule(t *testing.T, workspace string) string {
	t.Helper()
...
func initNestedModuleFixture(t *testing.T, req *Request) {
	t.Helper()
	workspace, err := os.MkdirTemp("", "kool-go-replace-nested-*")
	if err != nil {
		t.Fatal(err)
	}
	...
	consumer := initConsumerModule(t, workspace)
