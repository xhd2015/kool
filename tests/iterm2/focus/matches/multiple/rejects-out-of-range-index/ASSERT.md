## Expected

- An out-of-range index is fatal, does not focus, and includes both valid candidates.

```go
import "github.com/xhd2015/doctest/assert"

func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
 if err != nil { t.Fatal(err) }; if resp.ExitCode == 0 || len(resp.Focused) != 0 { t.Fatalf("exit=%d focused=%v", resp.ExitCode, resp.Focused) }
 assert.Output(t, resp.Stderr, `---
version: 3
---
Error: --index 2 is out of range \(valid indexes: 0\|1\)

  \[0\] window 1 \("credit-pricing"\) tab 2 session w0t2p0
  \[1\] window 3 \("worktrees"\) tab 1 session w2t1p0
`)
}
```
