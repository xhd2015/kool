...
- OS opener invoked with `vscode://xhd2015.open-in-new-window/open?path=...`.
...
Note: extension not reachable via IPC; opening via vscode:// URI.`)
	...
	if _, ok := parsed.Query()["replace"]; ok {
		t.Fatalf("fallback URI must not include replace query for default open, got %q", parsed.Query().Get("replace"))
	}
}
```